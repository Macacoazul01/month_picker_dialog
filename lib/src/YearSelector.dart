...
class YearSelector extends StatefulWidget {
	...
	const YearSelector({
		...
		this.firstDate,
		this.lastDate,
	}) : assert( initialDate != null ), assert( onYearSelected != null ), assert( upDownPageLimitPublishSubject != null ), assert( upDownButtonEnableStatePublishSubject != null ), super(key: key);
	@override State<StatefulWidget> createState() => YearSelectorState();
}
...
class YearSelectorState extends State<YearSelector> {
	...
	int _getPageCount() {
		if ( widget.firstDate != null && widget.lastDate != null ) {
			if ( widget.lastDate.year - widget.firstDate.year <= 12 ) return 1;
			else return  ( ( widget.lastDate.year - widget.firstDate.year + 1 ) / 12 ).ceil();
		} else if ( widget.firstDate != null && widget.lastDate == null ) {
			return ( _getItemCount() / 12 ).ceil();
		} else if ( widget.firstDate == null && widget.lastDate != null ) {
			return ( _getItemCount() / 12 ).ceil();
		} else return ( 9999 / 12 ).ceil();
	}
	...
	@override
	void initState() {
		_pageController = new PageController( initialPage: widget.firstDate == null ? ( widget.initialDate.year / 12 ).floor() : ( ( widget.initialDate.year - widget.firstDate.year ) / 12 ).floor() );
		super.initState();
		new Future.delayed(Duration.zero, () {
			widget.upDownPageLimitPublishSubject.add(
				new UpDownPageLimit(
					widget.firstDate == null ? _pageController.page.toInt() * 12 : widget.firstDate.year + _pageController.page.toInt() * 12,
					widget.firstDate == null ? _pageController.page.toInt() * 12 + 11 : widget.firstDate.year + _pageController.page.toInt() * 12 + 11,
				)
			);
			widget.upDownButtonEnableStatePublishSubject.add(
				new UpDownButtonEnableState(
